...
// Games are numbered in the order they become playable, so players don't
// have to play back-to-back and game N is always playable before game N+1.
export function computeGameNumbers(tournament: Tournament): Map<string, number> {
  const bracket = tournament.bracket
  const numbers = new Map<string, number>()

  const finalsGame1 = bracket.find(m => m.id === 'grand-finals') || null
  const finalsGame2 = bracket.find(m => m.id === 'grand-finals-2') || null
